...
for consensus_pref = 0:50:N
    for gridlock_pref = 0:50:N-consensus_pref
        ...
        for g = 1:G # run G games
            # Initialize game
            consensus_Chartists = Int(consensus_pref/2)
            gridlock_Chartists = Int(gridlock_pref/2)
            Consensus_makers = Int(consensus_pref/2)
            Gridlockers = Int(gridlock_pref/2)
            consensus_Zealots = Int(round((N-consensus_pref-gridlock_pref)/3))
            gridlock_Zealots = Int(round((N-consensus_pref-gridlock_pref)/3))
            party_Zealots = N-consensus_pref-gridlock_pref-consensus_Zealots-gridlock_Zealots
        ...
        output[Int(gridlock_pref/50)+1,Int(consensus_pref/50)+1] = avg_vote/G
    end
end
